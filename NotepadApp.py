...
class NotePadApp(Frame):
    ...
    def createWidgets(self):
        ...
        self.textPanel = PanedWindow(self, orient=HORIZONTAL)
        ...
        # text area
        self.textArea = Text(self.textPanel, undo=True, width=63, font=('Consolas', 16))
        self.textArea.pack(side=LEFT, expand=1, fill=BOTH)
        # self.textArea.grid(row=0, column=1)

        # scroll bar for text area
        self.scrollBar = Scrollbar(self.textPanel, width=40)
        ...
        # self.textPanel.add(self.lnLabel)
        # self.textPanel.add(self.textArea)
        # self.textPanel.add(self.scrollBar)
        self.textPanel.pack(expand=1, fill=BOTH)

        # Status bar
        self.status = Label(self, text='Status: ok', bd=1, relief='sunken', anchor=W)
